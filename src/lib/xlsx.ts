...
export function createXlsx(columns: string[], rows: unknown[][]) {
  const all = [columns, ...rows];
  const rowXml = all.map((row, rowIndex) => `<row r="${rowIndex + 1}">${row.map((value, columnIndex) => {
    const reference = `${columnName(columnIndex)}${rowIndex + 1}`; const style = rowIndex === 0 ? ' s="1"' : "";
